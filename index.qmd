...
"
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      # ---- Section 2: Sorting configuration (two columns) ----
      tags$span(class = "section-label", "Sorting configuration"),
      fluidRow(
        column(
          6,
          radioButtons(
            ...
            "Lag",
            c("3 months" = "3m", "6 months" = "6m", "Fama-French" = "ff"),
            selected = "3m"
          ),
          radioButtons(
            "breakpoints_main",
          ...
          radioButtons(
            ...
            "Exchanges",
            c("NYSE only" = "NYSE", "All" = "AMEX|NASDAQ|NYSE"),
            selected = "NYSE"
          )
        ),
        column(
          ...
          radioButtons(
            ...
            c(
              ...
              "Bivariate (dep.)" = "bivariate-dependent",
              "Bivariate (indep.)" = "bivariate-independent"
            ),
...
server <- function(input, output, session) {
  ...
  matched_id <- reactive({
    ...
    if (input$breakpoints_min_size == "NA") {
      res <- res %>% filter(is.na(breakpoints_min_size))
    } else {
      res <- res %>%
        filter(breakpoints_min_size == as.numeric(input$breakpoints_min_size))
    }
  ...
  factor_data <- reactive({
    ...
    bp_main <- input$breakpoints_main
    cache_file <- paste0(sv, "_", lag, "_", method, "_", bp_main, ".parquet")

    url <- sprintf(
      "https://huggingface.co/datasets/tidy-finance/factor-library/resolve/main/factor-library/sorting_variable=%s/sorting_variable_lag=%s/sorting_method=%s/breakpoints_main=%s/part-0.parquet",
      sv,
      lag,
      method,
      bp_main
