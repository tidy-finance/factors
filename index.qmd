...
grid_url <- paste0(
  ...
  "resolve/1d98a1742f135b94031e0925441508121e44a13e/portfolio_sort_grid.parquet"
)
...
server <- function(input, output, session) {
  ...
  factor_data <- reactive({
    ...
    url <- sprintf(
      "https://huggingface.co/datasets/tidy-finance/factor-library/resolve/42ca61426e06c8460dba39fcfe8431575134f2a9/sorting_variable=%s/sorting_variable_lag=%s/sorting_method=%s/n_portfolios_main=%s/part-0.parquet",
      sv,
      lag,
      method,
      bp_main
  ...
  # spacing around the assignment (" = " in R, "=" in Python) and `null_value`
  # / the boolean labels differ between the two languages.
  build_args <- function(sep, null_value, bool_labels) {
    sv <- input$sorting_variable

    args <- c(
      sprintf('sorting_variable%s"%s"', sep, sv),
      ...
      sprintf(
        'exclude_financials%s%s',
        sep,
        bool_labels[[as.character(input$exclude_financials)]]
      ),
      sprintf(
        'exclude_utilities%s%s',
        sep,
        bool_labels[[as.character(input$exclude_utilities)]]
      ),
      sprintf(
        'exclude_negative_earnings%s%s',
        sep,
        bool_labels[[as.character(input$exclude_negative_earnings)]]
      )
    )
